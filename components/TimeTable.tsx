            ...
            {intervals.map((intervalMinutes, rowIdx) => {
              ...
              return (
                <tr key={rowIdx}>
                  {isTopCell && (
                    <td
                      className="border border-gray-300 bg-gray-50 h-10 pt-1 pb-10 pr-1 text-end font-mono text-gray-600 select-none"
                      rowSpan={2}
                    >
                      {hour}
                    </td>
                  ...
                  {days.map((day, colIdx) => {
                    ...
                    if (course) {
                      ...
                      if (!timeSlot)
                        return <td key={colIdx} className="border border-gray-300 p-2 h-10 "></td>;
                      ...
                      return (
                        <td
                          key={colIdx}
                          className={`h-10 border border-gray-300 p-0.5 md:p-2 relative text-[10px] md:text-xs align-top ${assignColor(course.id)}`}
                          rowSpan={rowspanValue}
                        >
                          <h3 className="font-bold text-xs md:text-sm">
                            {infoHidden ? '' : course.sbjName}
                          </h3>
                          <p className="font-bold">{infoHidden ? '' : course.instructor}</p>
                          <p
                            className={`whitespace-pre-wrap wrap-break-word max-h-${4 * rowspanValue - 2} overflow-auto`}
                    ...
                    // 빈 셀 렌더링
                    return <td key={colIdx} className="border border-gray-300 p-2 h-10" />;
                  })}
                </tr>
              );
            })}
